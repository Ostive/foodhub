  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        visible && "bg-white/80 bg-neutral-950/80",
        className,
      )}
    >
      {children}
...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative hidden lg:flex flex-row items-center justify-center space-x-4 px-4 py-2 rounded-full bg-black/80 backdrop-blur-md text-white transition",
        className,
      )}
    >
      {items.map((item, idx) => (
        ...
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-gray-100 bg-neutral-800"
            />
          )}
...
export const MobileNavToggle = ({
  ...
  return isOpen ? (
    <IconX className="text-black text-white" onClick={onClick} />
  ) : (
    <IconMenu2 className="text-black text-white" onClick={onClick} />
  );
};
  ...
  return (
    ...
    >
      <img
        ...
        height={30}
      />
      <span className="font-medium text-black text-white">FoodHUB</span>
    </a>
  );
};
